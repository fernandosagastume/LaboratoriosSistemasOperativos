...
priorityDonation (struct thread *threadholder, int donation) 
{
    ...
    //Se realiza la donación al holder del lock
    ...
    if(l != NULL){
      struct thread *actualTH = l->holder;
      if(actualTH->priority < donation)
        priorityDonation(actualTH, donation);
    } 
    else {
...
void priorityDonationInverse(struct thread *currT, struct lock *lock){
  ...
  int maxPriority;
  if (!list_empty (&currT->holdingLocks)){
    maxPriority = list_max(&currT->holdingLocks, priorityCompareWaiters, NULL); 
  }

  if (currT->priority < maxPriority)
      currT->priority = maxPriority;
